...
pub struct AddMemberGroup<'info> {
    ...
    #[account(
        ...
        bump = epoch.bump,
        has_one = epoch_config @ ncashError::Unauthorized,
    )]
    ...
    #[account(
        constraint = signer.key().eq(&args.claim_info.context_address) @ ncashError::Unauthorized
    )]
...
pub fn add_member(ctx: Context<AddMemberGroup>, args: AddMemberGroupArgs) -> Result<()> {
    ...
    require!(
        ...
        ncashError::InvalidIdentifier
    );

    match group.members.binary_search(&context_address) {
        Ok(_) => return err!(ncashError::MemberAlreadyExists),
        Err(member_index) => {
            /* Selecting witnesses */
            ...
            for recovered_witness in recovered_witnesses {
                require!(
                    ...
                    ncashError::InvalidWitnessSignature
                );
            }
